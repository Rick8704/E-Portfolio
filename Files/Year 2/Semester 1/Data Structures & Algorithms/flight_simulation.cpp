...
// 2. TAN ZHI MING A23CS0148
// 3. NEO LI IN A23CS0253
// 4. ???
...
class Queue {
    private:
        ...
        int findInsertPosition(Flight* f);
...
void Queue::shiftLeft(int startIndex) {
...
int Queue::findInsertPosition(Flight* f) {
    for (int i = front; i <= rear; i++) {
        if (f->fuel < items[i]->fuel) {
            return i;
        }
    }
    return rear + 1;
